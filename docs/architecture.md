...
The deep dive: how the service is built, and the measurements behind every
decision in it. Start with the [README](../README.md) if you just want it
running — nothing here is required reading to deploy it.

This is a Graylog lookup-table backend (and Wazuh integration target) that
answers one question, fast:
...
- [Cross-worker sharing](#cross-worker-sharing)
- [Live stream](#live-stream)
- [Reconcile](#reconcile)
- [Request path](#request-path)
- [Hit semantics](#hit-semantics)
- [API contract](#api)
...
```mermaid
flowchart TB
    S["4 uvicorn workers start"] --> L{"flock LOCK_EX NB<br/>on membership.lock"}
    L -->|"wins"| B["<b>Builder</b><br/>bootstraps from OpenCTI, then publishes<br/>shm_name, count, generation to membership.json"]
    L -->|"loses"| R["<b>Readers</b><br/>poll for membership.json"]
    B --> P[("Shared memory segment<br/>one copy — 142 KB at 17.7k indicators")]
    R -->|"attach"| P
    R -.->|"attach timeout"| LV["Serve via the live backend,<br/>keep retrying in the background"]
    LV -.->|"segment appears"| P
    B -->|"every 24 h, or when<br/>the overlay fills"| RC["Reconcile — rebuild into<br/>generation + 1, old segment<br/>serves throughout"]
    RC --> P
```
...
| Path | Use it for |
|---|---|
| **[Graylog lookup tables](graylog-setup.md)** | High-volume streams. No process spawn; Graylog's own cache absorbs repeats. |
| **[Wazuh integration](wazuh.md)** | Targeted enrichment inside Wazuh's correlation — FIM hashes, specific rule groups. |
...
Setup steps live in [graylog-setup.md](graylog-setup.md),
[pipeline-rules.md](pipeline-rules.md) and [wazuh.md](wazuh.md).
